        ...
        <section class="panel">
          <table class="table table-striped table-advance table-hover">
            <tbody>
              ...
              @foreach ($ad as $ad)
              <tr>
              <td ><img class="img-circle" height="50" width="50" src="{{$ad->photo ? $ad->photo->file.$ad->photo->path : 'img/avatar1.jpg' }}"  alt=""></td>
                <td>{{$ad->name}}</td>
                <td>{{$ad->created_at->diffForHumans()}}</td>
                <td>{{$ad->updated_at->diffForHumans()}}</td>
                <td>
                    {!! Form::open(['method'=>'DELETE','action'=>['Advirtisement@destroy',$ad->id]] )!!}
                  <div class="btn-group">
                  <a class="btn btn-primary" href="{{route('advirtisement.edit',$ad->id)}}"><i class="icon_pencil-edit"></i></a>                 
